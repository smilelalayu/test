...
public class CnConcurrentSessionFilter extends GenericFilterBean {
	...
	public CnConcurrentSessionFilter(SessionRegistry sessionRegistry) {
			...
			if (info != null) {
				if (info.isExpired()) {
					...
					final List<SessionInformation> sessions = sessionRegistry
							.getAllSessions(info.getPrincipal(), false);

					SessionInformation latest = null;
					for (SessionInformation sessioninfo : sessions) {
						if ((latest == null)
								|| sessioninfo.getLastRequest().after(
										latest.getLastRequest())) {
							latest = sessioninfo;
						}
					}
					GxtSessionUser user = (GxtSessionUser) (latest
							.getPrincipal());

					String ip = user.getIp();

					System.out.println("最新登录ip地址：" + ip);
