  ...
  /**
	...
	public static int subDate(Date date1,Date date2) {
	  ...
	  /**
		 ...
		 * 
		 */
		public static int subMin(Date date1,Date date2) {
			
			long timeNow = date1.getTime();
			long timeOld = date2.getTime();
			long lngSub = (timeNow - timeOld) / (1000 * 60);
			if(lngSub<0){
				return 0;
			}else {
				return (int)lngSub;
...
/*  
  public static void main(String[] args)
  {
     ...
     * 把传入的时间设置为只有年月日:
     */
    public static Date getSimpleDate(Date current) {
    	Calendar c = Calendar.getInstance();
    	c.setTime(current);
