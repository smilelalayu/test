...
//		List<LnCommodityInfo> infoList = lnCommodityInfoService.list(commodity);
	...
	private void setCalcInfo(LnAppModel model,LnApp lnApp) throws Exception {
		...
		plan.setPrincipal(lnApp.getLoanMoney());
		//获取还款日期
		if (lnApp.getSignTime() == null) {
			plan.setPayDate(new Date());
		} else {
			plan.setPayDate(lnApp.getClientSignedTime());// 根据合同签订时间推算还款日期
		}
		*//**
		 * 从工单数据上获取还款计划所需参数
		 *//*
		plan.setTerms(lnApp.getPeriods());
		if(!InputChecker.isEmpty(lnApp.getContrAnnualInterestRate())){
		   plan.setYearRate(lnApp.getContrAnnualInterestRate().divide(oneHundred));
		}
		if(!InputChecker.isEmpty(lnApp.getInterestRate())){
			plan.setTotRate(new BigDecimal(lnApp.getInterestRate()).divide(oneHundred));
		}
		
         if(!InputChecker.isEmpty(lnApp.getDiscountRate())){
        	 plan.setTrDiscount(lnApp.getDiscountRate().divide(oneHundred));
		}      
		plan.setPchIns(lnApp.getBuyInsurance());// 是 = true ，否= false
        if(!InputChecker.isEmpty(lnApp.getMonthlyPremiumRate())){
        	plan.setInsRate(lnApp.getMonthlyPremiumRate().divide(oneHundred));
		}        
		
        if(!InputChecker.isEmpty(lnApp.getReserveRate())){
    	   plan.setInfoFeeRate(lnApp.getReserveRate().divide(oneHundred));
		}
        if(!InputChecker.isEmpty(plan.getYearRate())){
        	RepayPlanCalculator.calcICFee(plan);
    		plan.setMonthlyFee(plan.getMonthlyFee().setScale(2, BigDecimal.ROUND_HALF_UP));
    		plan.setInfoFee(plan.getInfoFee().setScale(2, BigDecimal.ROUND_HALF_UP));
    		plan.setConsultFee(plan.getConsultFee().setScale(2, BigDecimal.ROUND_HALF_UP).multiply(oneHundred));
 		}
